...
import { Penilaian, SubCriteria, Criteria, Candidate, TabelBobotGap } from "db"
...
export const ProfileMatchingExec = (
  penilaian: PenilaianInput = [],
  criteria: CriteriaInput = [],
  tableBobot: TabelBobotGap[] = []
): ProfileMatchType => {
    ...
    _.forEach(groupByType, function (subCiteria, key) {
      subCiteria.map((subCriteriaDat) => {
        subCriteriaDat.listCandidate.map((listCandidateDat) => {
          const exiestDat = _.find(avgTotalCriteria.listCandidate, {
            ...
            if (key == "SECOND_FACTOR") {
              avgTotalCriteria.listCandidate.map((data) => {
                if (data.nama == exiestDat.nama) {
                  ...
                  data.secondFactor =
                    data.secondFactorDesc.reduce((x, y) => x + y) / data.secondFactorDesc.length
                  const valCriteria = _.find(criteria, { nama: datCriteria.namaCriteria })
                  data.nilaiTotal =
                    (valCriteria!.coreFactor / 100) * data.coreFactor +
                    (valCriteria!.secondaryFactor / 100) * data.secondFactor
                }
              })
            } else {
              avgTotalCriteria.listCandidate.map((data) => {
                if (data.nama == exiestDat.nama) {
                  ...
                  data.coreFactor =
                    data.coreFactorDesc.reduce((x, y) => x + y) / data.coreFactorDesc.length
                  const valCriteria = _.find(criteria, { nama: datCriteria.namaCriteria })
                  data.nilaiTotal =
                    (valCriteria!.coreFactor / 100) * data.coreFactor +
                    (valCriteria!.secondaryFactor / 100) * data.secondFactor
  ...
  result.ranking = _.sortBy(result.ranking, "value").reverse()
